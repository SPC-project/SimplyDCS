...
class MyWindow(QMainWindow):
    ...
    def f(self,t):
        # self.listWidget.addItem("plot function: " + function)
        
        return 5*sin(4*t)
    

    def plot_function(self,function):
        # self.listWidget.addItem("function: " + function)
        r = arange(0.0, 5.0, 0.1)
        t = sympy.symbols('t')
        # t2 = np.arange(0.0, 5.0, 0.02)
        # x = sympy.sympify(function)
        parse = sympy_parser.parse_expr(function)
        # p = str(parse)
        # self.listWidget.addItem("x: " + p)
        
        ar = []
        for i in r:

            element = parse.subs({t:i})
            # print(element)
            ar.append(element)
            # print(ar)

        plt.figure(1)
        # self.listWidget.addItem("x.subs({t:r}): " + parse.evalf({t:r}))
        plt.plot(ar, 'r--')
        plt.show()
